...
const AssessmentContent = ({ setCurrentView }) => {
  ...
  const progress = Math.round(((currentIdx + 1) / assessmentQuestions.length) * 100);
        ...
        <div className="bg-[#0F172A] border border-slate-800/50 rounded-[32px] p-8">
              ...
              return (
                <div key={domain} className="flex items-center space-x-3">
                  <div className={`h-2 w-2 rounded-full transition-all duration-500 ${
                    isCurrent ? 'bg-blue-500 shadow-[0_0_8px_#3b82f6]' : isPast ? 'bg-emerald-500' : 'bg-slate-800'
                  }`} />
          ...
          <div className="mt-8 pt-8 border-t border-slate-800/50">
            <div className="flex justify-between items-end mb-2">
              ...
              <span className="text-blue-500 font-black text-xs">{progress}%</span>
            </div>
        ...
        <div className="bg-[#0F172A] border border-slate-800/50 rounded-[40px] p-12 relative min-h-[500px] flex flex-col justify-between overflow-hidden">
          ...
          {isSubmitting ? (
            <div className="flex-1 flex flex-col items-center justify-center text-center space-y-6">
              ...
              <h2 className="text-2xl font-black text-white tracking-tight">Finalizing Risk Matrix...</h2>
              <p className="text-slate-500 font-mono text-[10px] uppercase tracking-[0.3em]">Uploading encrypted node data to SME-Cloud</p>
            </div>
          ) : (
            <>
              <div className="space-y-8">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-slate-600 font-mono text-[10px] uppercase tracking-widest">Protocol v4.0.1</span>
                </div>

                <div className="space-y-4">
                  <h4 className="text-blue-500 font-black text-[11px] uppercase tracking-[0.3em] italic">{currentQuestion.domain}</h4>
                ...
                <div className="grid grid-cols-1 gap-4 pt-6">
                  {currentQuestion.options.map((opt) => {
                    ...
                    return (
                      ...
                      >
                        <div>
                          <p className={`font-bold transition-colors ${isSelected ? "text-white" : "text-slate-300 group-hover:text-white"}`}>
                            {opt.label}
                          </p>
                          <p className={`text-[10px] mt-1 font-medium ${isSelected ? "text-blue-100" : "text-slate-500"}`}>
                            {isSelected ? "Selection confirmed" : "Click to select response"}
                          </p>
                        </div>
                        <div className={`h-6 w-6 rounded-full border-2 flex items-center justify-center transition-all ${
                          isSelected ? "bg-white border-white" : "border-slate-700 group-hover:border-slate-500"
                        }`}>
